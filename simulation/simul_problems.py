...
class KineticSimulationProblem(SimulationProblem):
    ...
    def simulate_with_flux(self, overrideSimulProblem=None, calc_flux=False):
        """
        ...
        if self.timeout is None:
            status, sstateRates, sstateConc, Conc = _my_kinetic_solve_with_flux(self.get_model(), self.parameters,
                                                                final_factors,
                                                                initConcentrations,
                                                                self.get_time_steps())
            if calc_flux:
                flux=self.calculate_flux(Conc,overrideSimulProblem)
            else:
                flux={}
        else:
            p = MyPool(processes=1)
            res = p.apply_async(_my_kinetic_solve_with_flux, (
                self.get_model(), self.parameters, final_factors, initConcentrations,
                self.get_time_steps()))
            try:
                status, sstateRates, sstateConc, Conc = res.get(self.timeout)  # Wait timeout seconds for func to complete.
                if calc_flux:
                    flux=self.calculate_flux(Conc)
            except Exception:
        ...
        return kineticSimulationResult(self.model.id, solverStatus=status, ssFluxesDistrib=sstateRates,
                                       ...
                                       overrideSimulProblem=overrideSimulProblem, timeConc=Conc, timeFlux=flux)

    def calculate_flux(self,conc,overrideSimulProblem=None):
        final_factors = {}
        if overrideSimulProblem:
            final_factors = overrideSimulProblem.factors
            factorsParam = OrderedDict([(k,v) for k, v in final_factors.items()])
            rates = OrderedDict()
            for rId in self.get_model().reactions.keys():
                newExp = self.get_model().parsedRates[rId]
                if rId in self.get_model().reacParamsFactors.keys():
                    toModify = set(factorsParam.keys()).intersection(self.get_model().reacParamsFactors[rId])
                    if len(toModify) > 0:
                        for elem in toModify:
                            newExp = re.sub(r"([pv]\['" + elem + "'\])", str(factorsParam[elem]) + r" * \1", newExp);
                rates[rId] = newExp
            p=self.get_model().get_parameters()
            r=self.get_model().parsedRules
            ...
            try:
                go=0
                while go!=len(conc.iloc[:,0]):
                    t=time[go]
                    for i in range(len(conc.iloc[go,:])):
                        x[i]=conc.iloc[go,i]
                    for k in r.keys():
                        v[k]=eval(r[k])
                    for j in rates.keys():
                ...
                print("Error in calculate_flux: " +str(e))
            reactions=pd.DataFrame(flux,self.tSteps)
            reactions.index.name="Time"
            return (reactions)
        else:
            rates=self.get_model().parsedRates
            p=self.get_model().get_parameters()
            r=self.get_model().parsedRules
            time=self.get_time_steps()
            v=OrderedDict()
    ...
    def simulate(self, overrideSimulProblem=None):
        """
        This method performs the phenotype simulation of the kinetic model, using the solverId method and applying the modifications present in the instance of overrideSimulProblem.
...
def _my_kinetic_solve_with_flux(model, finalParameters, finalFactors, initialConc, timePoints):
    """
    ...
    try:
        ...
        for c  in X[:]:
            for k in c:
                if k < -1*SolverConfigurations.RELATIVE_TOL:
    ...
    final_conc = OrderedDict(zip(model.metabolites.keys(), X[-1]))
    rez = [[X[j][i] for j in range(len(X))] for i in range(len(X[0]))] 
    concentrations = OrderedDict(zip(model.metabolites.keys(), rez))
    conc=pd.DataFrame(concentrations,t)
    conc.index.name="Time"
    return solverStatus.OPTIMAL, finalRates, final_conc, conc
